...
void displayAllContacts (vector <Contact> &addressBook)
{
	for (int i = 0; i < addressBook.size(); i++)
	{
		...
		cout << "Nacisnij enter, aby przejsc do kolejnego wyswietlenia. Brak wyswietlania danych oznacza brak kontaktow." << endl;
	...
	while(true)
        {
			...
			for (itr; itr != addressBook.end(); ++itr)
			{
				if (choice == itr-> contactId)
				{
					...
					cout << "Czy na pewno chcesz usunac uzytkownika? Jesli tak wcisnij t. "<<endl;
