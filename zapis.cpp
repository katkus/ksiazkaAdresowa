...
void saveAllContacts (vector <Contact> &addressBook, int&loggedInUserId)
{
	...
	if (plik.good() == true)
	{
                ...
                /*      case 5:
                        ...
                        if (position >= 4)
                        {
				fstream zapis_pliku;//przenieść to na poczatek pliku?
				...
				if (zapis_pliku.good() == true)
				{
						...
						zapis_pliku << contacts.surname << '|' <<endl;
						...
						for (int i = 0; i < addressBook.size(); i++ )
						{
							if(contacts.contactId == addressBook[i].contactId)
							{
								zapis_pliku << addressBook[i].contactId << '|';
                                                		...
                                                		zapis_pliku << addressBook[i].surname << '|' <<endl;
		...
		return;
                ...
                if (rename (temporaryContactFile.c_str(), contactFile.c_str()) == 0 )
                {
                        cout<<"Zmiana nazwy pliku powiodla sie."<<endl;
                }
                else
                {
                        cout<<"Zmiana nazwy nie powiodla sie."<<endl;
                }
sleep(3);
        return;
}
...
void editContact (vector <Contact> &addressBook, int &lastContactId, int&loggedInUserId)
{
	...
	if( addressBook.size() == 0)
	{
		...
		{
			cout<< "Wprowadzono nieprawidlowe dane. Zostaniesz cofniety do wczesniejszego kokpitu." << endl; sleep(2); return;
		}
	}
return;
}
...
void addNewContact (vector <Contact> &addressBook, int &lastContactId, int &loggedInUserId)
{
        ...
        while (i < addressBook.size())
        {
                 ...
                 {
                         cout << "Kontakt o podanym imieniu i nazwisku juz istnieje. Prosze o podanie nowej nazwy użytkownika." << endl;
                         cout << "Podaj imie: ";
                         auxiliaryFirstName = readData();
                         cout << "Podaj nazwisko: ";
...
fstream plik_do_odczytu (contactFile.c_str());
		...
		sleep(2);
         ...
         cout << "Brak kontaktow. /Kontakt zostal dodany. " << endl;
         sleep(2);
         return;
 }
...
void searchByFirstName (vector <Contact> &addressBook)
{
        ...
        cout << "Nacisnij enter, aby przejsc do kolejnego wyswietlenia. Brak wyswietlania danych oznacza brak kontaktow o danym imieniu." << endl;
...
void searchBySurname (vector <Contact> &addressBook)
{
        ...
        cout << "Nacisnij enter, aby przejsc do kolejnego wyswietlenia. Brak wyswietlania danych oznacza brak kontaktow o danym nazwisku."<<endl;
        getchar();
}
...
void logIn (vector <User> &userList)
{
        ...
        while (i < userList.size())
        {
                if (userList[i].username == username)
                {
                        ...
                        cout << "Podałeś 3 razy błędne hasło. Poczekaj 3 sekundy przed kolejną proba."<<endl;
                        sleep(3);
                        return;
                }
                i++;
